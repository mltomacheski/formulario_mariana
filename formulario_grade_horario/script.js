var meuFormulario = document.getElementById("grade_de_horario");
meuFormulario.addEventListener("submit", function(event) {
  event.preventDefault();
  grade_de_horario();
});

function capturarAulasSemanais() {
  const primeiro_numero_inteiro = Number(document.getElementById("dia_semana").value);
  return primeiro_numero_inteiro;
}

function capturarPosiçãodeAula() {
 const segundo_numero_inteiro = Number(document.getElementById("posicao_aula").value); 
